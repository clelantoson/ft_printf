...
/*   Updated: 2021/03/30 18:03:14 by cle-lan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_deal_with_str(char *str, t_flags *data)
{
	...
	else if (data->dot > 0 && (size_t)data->dot < ft_strlen(str)) //".2s" "hello"
	{
		data->dot = 0;
		ft_deal_with_dot(str, data);
	}
	// if (data->bool_dot > 0) //".5s"
	// 	ft_deal_with_dot(str, data);
	if (data->bool_dot == 1 && data->dot == 0) //".s"
	{
		ft_deal_with_dot(str, data);
	}
}
